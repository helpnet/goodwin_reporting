...
def buildRecordList(num_record, interval, opener):
    global ticket_hold, list_problem
...
def pullData(url, opener):
    global list_stu, list_problem, soe_course, sotaps_course
    ...
    if not name in list_stu:
        list_stu[name] = [os, browser, [course_id], java_version, topic, date_open]

    if not course_id in list_stu[name][2]:
        list_stu[name][2].append(course_id)

    myList = [url, date_open, name, email, topic, ticket_content, os, browser, course_id, java_version, college]
